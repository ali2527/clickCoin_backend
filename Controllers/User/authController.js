...
exports.signup = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      return res
        .status(400)
        .json(ApiResponse({}, "Creator with this Email / Username Already Exist", false));
    }
    ...
    const title ="New Creator Signup"
    const content = `A new creator has signed up on the app. Name : ${firstName + " " + lastName}, Email : ${email}`
    sendNotificationToAdmin(title,content)   

    return res
      .status(200)
      .json(ApiResponse({ user }, "Creator Created Successfully", true));
  } catch (error) {
  ...
  try {
    User.findOne({ email })
      .then((user) => {
        if (!user) {
          return res.json(
            ApiResponse({}, "Creator with this email not found", false)
          );
        }
...
exports.emailVerificationCode = async (req, res) => {
  try {
    ...
    `;
    await generateEmail(email, "Click Coin - Password Reset", html);
